...
if(isset($_GET["Dependent_id"]) && !empty(trim($_GET["Dependent_id"]))){
    ...
    $sql1 = "SELECT * FROM DEPENDENT_EC WHERE Essn = ? AND Dependent_id = ?";
  
    if($stmt1 = mysqli_prepare($link, $sql1)){
        // Bind variables to the prepared statement as parameters
        mysqli_stmt_bind_param($stmt1, "is", $param_Essn, $param_Dependent_id);      
        // Set parameters
       $param_Essn = $_SESSION["Ssn"];
       $param_Dependent_id = $_SESSION["Dependent_id"];
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($Dependent_name_err) && empty($Relationship_err) && empty($Bdate_err) && empty($Sex_err)){
        ...
        $sql = "UPDATE DEPENDENT_EC SET Essn=?, Dependent_name=?, Sex=?, Bdate=?, Relationship=? WHERE Dependent_name=?";
